      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-outline-variant shadow-sm space-y-5">
            <div className="flex justify-between items-center pb-3 border-b border-outline-variant">
              ...
              <span className="text-xs font-bold font-mono bg-primary/10 text-primary px-2.5 py-1 rounded-lg">{properties.length} Portfolios</span>
            </div>

            {properties.length === 0 ? (
              <div className="text-center py-10 text-on-surface-variant">
                <Building className="w-12 h-12 mx-auto text-on-surface-variant/30 mb-3" />
                <p className="text-sm font-bold font-mono">No properties drafted.</p>
              </div>
            ) : (
                ...
                {properties.map(prop => {
                  ...
                  return (
                    <div key={prop.id} onClick={() => { setSelectedPropId(prop.id); setFilterStatus("All"); setSelectedBlockId("All"); }}
                      className={`group cursor-pointer p-5 rounded-xl transition-all border-2 ${isActive ? "bg-primary/5 border-primary shadow-md shadow-primary/5" : "bg-white hover:bg-slate-50 border-outline-variant hover:border-primary/20 hover:shadow-sm"}`}>
                      <div className="flex items-start justify-between">
                        <div className="flex gap-3.5 min-w-0 flex-1">
                          <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${isActive ? "bg-primary/10 text-primary" : "bg-slate-100 text-on-surface-variant"}`}>
                            <Building className="w-5 h-5" />
                          </div>
                          <div className="min-w-0">
                            <h4 className="font-extrabold text-sm text-on-surface truncate">{prop.name}</h4>
                            <p className="text-xs font-mono font-medium text-on-surface-variant flex items-center gap-1.5 mt-1">
                              <MapPin className="w-3.5 h-3.5 text-primary shrink-0" />
                              <span className="truncate">{prop.location}</span>
                            </p>
                          </div>
                        </div>
                        <ChevronRight className={`w-4 h-4 transition-transform duration-200 shrink-0 ml-2 ${isActive ? "text-primary translate-x-1" : "text-on-surface-variant opacity-40 group-hover:opacity-100"}`} />
                      </div>
                      <div className="mt-4 grid grid-cols-2 gap-3 border-t border-outline-variant/60 pt-3 text-xs">
                        <div>
                          <p className="text-[10px] font-bold font-mono text-on-surface-variant uppercase">Units</p>
                          <p className="font-bold text-on-surface mt-1">{propUnits.length} registered</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-bold font-mono text-on-surface-variant uppercase">Occupancy</p>
                          <p className="font-bold text-on-surface mt-1">{propUnits.length > 0 ? `${occupancy}%` : "No units"}</p>
                        </div>
                      </div>
                      {propUnits.length > 0 && (
                        <div className="mt-3 w-full bg-slate-100 rounded-full h-1.5">
                          <div className={`h-1.5 rounded-full transition-all duration-500 ${occupancy >= 80 ? "bg-emerald-500" : occupancy >= 50 ? "bg-amber-500" : "bg-primary"}`} style={{ width: `${occupancy}%` }} />
                        </div>
                      )}
                    </div>
                  );
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="bg-white rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
            {/* Header */}
            <div className="p-7 border-b border-outline-variant flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <span className="text-[10px] font-bold font-mono text-primary uppercase bg-primary/10 px-2.5 py-1 rounded-lg">
                  {currentProp?.name || "Select a property"}
                </span>
                <h3 className="font-extrabold text-xl tracking-tight text-on-surface mt-2">Active Rental Units</h3>
                <p className="text-sm text-on-surface-variant mt-1">
                  Showing <span className="font-bold">{filteredUnits.length}</span> of <span className="font-bold">{currentUnits.length}</span> units configured under this property.
                </p>
              </div>
              <button onClick={() => {
              ...
              }} className="px-5 py-2.5 bg-primary text-white hover:bg-primary-hover rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm">
            ...
            {currentPropBlocks.length > 0 && (
              <div className="px-7 py-3.5 border-b border-outline-variant bg-slate-50 flex flex-wrap items-center gap-2.5">
                <span className="text-[10px] font-bold font-mono text-on-surface-variant uppercase tracking-wider flex items-center gap-1.5 shrink-0">
                  <Layers className="w-3.5 h-3.5 text-primary" /> Block Filter:
                </span>
                <button onClick={() => setSelectedBlockId("All")}
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-bold border transition-all ${selectedBlockId === "All" ? "bg-white border-primary text-primary shadow-xs" : "bg-transparent border-transparent text-on-surface-variant hover:text-primary"}`}>
                  All Blocks
                </button>
                {currentPropBlocks.map(block => {
                  const isSelected = selectedBlockId === block.id;
                  return (
                    <div key={block.id} className={`inline-flex items-center gap-1.5 pl-3.5 pr-2 py-1.5 rounded-lg text-xs font-bold border transition-all ${isSelected ? "bg-white border-primary text-primary shadow-xs" : "bg-transparent border-transparent text-on-surface-variant hover:text-primary hover:bg-slate-100/50"}`}>
                ...
                <button onClick={() => setSelectedBlockId("None")}
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-bold border transition-all ${selectedBlockId === "None" ? "bg-white border-primary text-primary shadow-xs" : "bg-transparent border-transparent text-on-surface-variant hover:text-primary"}`}>
                  No Block
                </button>
              </div>
            )}
            ...
            {/* Status Filter */}
            <div className="px-7 py-3.5 border-b border-outline-variant flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-bold font-mono text-on-surface-variant uppercase tracking-wider shrink-0 mr-1">Status:</span>
              {(["All", "Vacant", "Reserved", "Occupied", "Notice Given", "Under Maintenance"] as const).map(status => (
                <button key={status} onClick={() => setFilterStatus(status)}
                  className={`px-3.5 py-1.5 text-xs font-bold rounded-full border transition-all ${filterStatus === status ? "bg-primary text-white border-primary shadow-xs" : "bg-white border-outline-variant text-on-surface-variant hover:border-primary/40 hover:text-primary"}`}>
                  {status}
                </button>
              ))}
            </div>
            ...
            <div className="overflow-x-auto">
              {filteredUnits.length === 0 ? (
                <div className="p-16 text-center text-on-surface-variant">
                  <Building className="w-14 h-14 mx-auto stroke-1 opacity-30 mb-4" />
                  <p className="text-base font-bold text-on-surface">No units in this filter</p>
                  <p className="text-sm text-on-surface-variant mt-1.5">Change block criteria or add a unit to get started.</p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-50">
                    <tr className="border-b border-outline-variant">
                      <th className="px-7 py-4 text-xs font-bold font-mono text-on-surface-variant uppercase">Unit Code</th>
                      <th className="px-7 py-4 text-xs font-bold font-mono text-on-surface-variant uppercase">Block</th>
                      <th className="px-7 py-4 text-xs font-bold font-mono text-on-surface-variant uppercase">Status</th>
                      <th className="px-7 py-4 text-xs font-bold font-mono text-on-surface-variant uppercase text-right">Monthly Rent</th>
                      <th className="px-7 py-4 text-xs font-bold font-mono text-on-surface-variant uppercase text-center">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {filteredUnits.map(unit => {
                      ...
                      return (
                        <tr key={unit.id} className="hover:bg-primary/[0.02] transition-colors">
                          <td className="px-7 py-5 font-bold text-sm text-on-surface">
                            <div className="flex items-center gap-2.5"><Building className="w-4 h-4 text-primary shrink-0" /><span>{unit.unit_code}</span></div>
                          </td>
                          <td className="px-7 py-5 text-xs font-mono font-medium text-on-surface-variant">
                            {associatedBlock ? (
                              <span className="bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">{associatedBlock.name}</span>
                            ) : (
                              <span className="text-slate-400 font-normal">No Block</span>
                            )}
                          </td>
                          <td className="px-7 py-5">
                            <select value={unit.status} onChange={e => handleUpdateUnitStatus(unit.id, e.target.value as Unit["status"])}
                              className={`px-3 py-1.5 border text-xs font-bold font-mono rounded-full focus:outline-none focus:ring-1 focus:ring-primary ${getStatusColor(unit.status)}`}>
                          ...
                          <td className="px-7 py-5 font-mono font-bold text-sm text-on-surface text-right">KSh {unit.rent_amount.toLocaleString()}</td>
                          <td className="px-7 py-5 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              <button onClick={() => handleDeleteUnit(unit.id, unit.unit_code)} className="p-2 rounded-lg text-on-surface-variant hover:text-red-650 hover:bg-red-50 transition-colors" title="Remove Unit">
